...
pygame.draw.circle(screen,(0,255,0),(start.x,start.y),5,width=0)
pygame.draw.circle(screen,(255,0,0),(goal.x,goal.y),5,width=0)
...
while running:
    ...
    if not path_found:
	    new_node=sample_closest(tree)
	    tree.append(new_node)
	    pygame.draw.circle(screen,(0,0,0),(new_node.x,new_node.y),3,width=0)
	    dist=((new_node.x-goal.x)**2 + (new_node.y-goal.y)**2)**0.5
	    if dist<20:
    		final_node=new_node
    		path_found=True
